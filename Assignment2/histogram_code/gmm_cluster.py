...
def inv(cov_mat,d):
	...
	for i in range(d):
		if abs(cov_mat[i][i])==0:
			inv_cov[i][i]=1
			#if cov_mat[i][i] < 0 :
			#	inv_cov[i][i]=-10
			
		#elif abs(cov_mat[i][i]) > 10000:
		#		inv_cov[i][i]=1
		#		if cov_mat[i][i] < 0 :
		#			inv_cov[i][i]=-0.0001
...
def det(cov_mat,d):
	...
	for i in range(d):
		if abs(cov_mat[i][i]) != 0:
				dett=dett*(np.power(abs(cov_mat[i][i]),(1.0/3)))
...
def G(pos,mu,cov_matrix,d):        
	...
	fac=np.power(fac,(1.0/5))
	#fac=fac/100000
	#print(-fac/2)
	#print(pos-mu)
	#print("fac=",fac)
	#print("exp=",np.exp(-fac ))
	#if np.exp(-fac ) < 0.00000000000001:
		#print("verysmall")
	#if 	np.exp(-fac ) > 100000000000000:
		#print("verylarge")
	#print("b=",np.exp(-fac))
	return np.exp(-fac)/N;
...
def Gmm(x,n,d,k,Z_nk,Pi_k,mean,cov_mat,cn,xg):
	...
	cy=0
	epoch=0
	while cy < 2 or (l_new-l_old)>0.001 :
		cy=cy+1
		l_old=l_new
		l_new=0.0000
		total_sum=0.0
