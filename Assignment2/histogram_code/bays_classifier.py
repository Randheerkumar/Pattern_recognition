...
def class_f(x,n,mean1,cov1,Pi_k1,mean2,cov2,Pi_k2,mean3,cov3,Pi_k3,k,d,cn,conf):
	...
	# nt=int(raw_input("enter number of test clases :"))	
		...
		for i in range(n):
			temp=np.zeros(3)
			for j in range(k):
				temp[0] += (Pi_k1[j]*gmm_cluster.G(x[i],mean1[j],cov1[j],d))
			for j in range(k):
				temp[1] += (Pi_k2[j]*gmm_cluster.G(x[i],mean2[j],cov2[j],d))
			for j in range(k):
				temp[2] += (Pi_k3[j]*gmm_cluster.G(x[i],mean3[j],cov3[j],d))

			#print(temp)
			'''
			if temp[0]!=0.000000:
				temp1[0]+=np.log(1+temp[0])
			if temp[1]!=0.000000:	
				temp1[1]+=np.log(1+temp[1])
			if temp[2]!=0.000000:	
				temp1[2]+=np.log(1+temp[2])
			'''
			index=np.argmax(temp)
			arr[index]+=1
		print(arr)	
		conf[cn][np.argmax(arr)]+=1
